...
- no-follow 文件系统访问、cap-style root walking、atomic write 与 bounded file read
- host command 探测、执行与 process-tree cleanup building blocks
...
secret-kit            -> omne-fs-primitives
secret-kit            -> omne-process-primitives
secret-kit            -> structured-text-kit

omne-fs-primitives   -> (no internal foundation deps)
omne-process-primitives -> (no internal foundation deps)
...
- `omne-fs-primitives` 与 `omne-process-primitives` 现在作为 workspace 内部 host/runtime building blocks 存在，供更高层 crate 复用；它们属于 foundation 的低层原语，而不是从外部 runtime workspace 反向借用。
...
- `text-assets-kit` 刻意不依赖 `i18n-kit`，并只通过 `omne-fs-primitives` 复用底层文件系统原语，保持通用文本资源/runtime fs adapter 边界。
...
- `i18n-kit` 依赖的是结构化文本原语；`secret-kit` 额外复用 `error-kit` 以暴露稳定错误语义，并通过 workspace 内部 process/fs primitives 复用主机侧 building blocks。
