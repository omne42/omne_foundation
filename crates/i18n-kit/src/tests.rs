...
use structured_text_kit::{StructuredText, try_structured_text};
...
fn renders_nested_structured_text_with_catalog() {
    ...
    let inner =
        try_structured_text!("inner", "name" => "ditto").expect("literal arg name should be valid");
    let text =
...
fn resolve_cli_locale_rejects_empty_equals_value() {
    ...
    let error =
        resolve_locale_from_cli_args(&*CATALOG, vec!["--locale=".to_string()], "DITTO_LOCALE")
...
fn resolve_cli_locale_rejects_flag_after_first_positional_after_program_name() {
    ...
    static CATALOG: LazyLock<StaticJsonCatalog> = LazyLock::new(|| {
        StaticJsonCatalog::try_new(Locale::from_static("ca_ES"), &SOURCES).expect("valid catalog")
    });
...
fn global_catalog_uninitialized_lookup_is_a_miss() {
    ...
    assert_eq!(
        catalog.try_resolve_locale(None),
        Err(ResolveLocaleError::CatalogNotInitialized)
    );
    assert_eq!(
        resolve_locale_from_argv(&catalog, vec!["cmd".to_string()], "DITTO_LOCALE"),
        Err(ResolveLocaleError::CatalogNotInitialized)
    );
    assert_eq!(
        catalog.render_text(
...
#[test]
fn composed_catalog_skips_uninitialized_global_catalog_miss() {
    let catalog = ComposedCatalog::new(Locale::EN_US)
        .add_catalog(GlobalCatalog::new(Locale::EN_US))
        .add_catalog(
            StaticJsonCatalog::try_new(
                Locale::EN_US,
                &[StaticJsonLocale::new(
    ...
    assert_eq!(
        catalog.get_text(Locale::EN_US, "hello_name"),
...
fn global_catalog_replace_does_not_wait_for_inflight_reads() {
    ...
    impl TranslationCatalog for FastCatalog {
        fn resolve_shared(&self, _locale: Locale, _key: &str) -> TranslationResolution {
            TranslationResolution::Exact(Arc::<str>::from("fast"))
        }
    }

    impl Catalog for FastCatalog {
        fn lookup_shared(&self, locale: Locale, key: &str) -> Option<Arc<str>> {
            self.get_text_shared(locale, key)
        }
    ...
    let reader = thread::spawn(move || {
        assert_eq!(
            reader_catalog.get_text(Locale::EN_US, "hello"),
            Some("slow".to_string())
        );
    });
    ...
    entered_rx
        ...
        .expect("reader should enter catalog");

    let replacer_catalog = Arc::clone(&catalog);
    let (replace_done_tx, replace_done_rx) = mpsc::channel();
    let replacer = thread::spawn(move || {
        replacer_catalog.replace(FastCatalog);
        replace_done_tx.send(()).expect("signal replace done");
    });

    let replace_completed = replace_done_rx
        .recv_timeout(Duration::from_millis(200))
        .is_ok();

    release_tx.send(()).expect("release reader");
    reader.join().expect("join reader thread");
    replacer.join().expect("join replace thread");

    assert!(
        replace_completed,
        "replace should not wait on in-flight reads"
    );
    assert_eq!(
        catalog.get_text(Locale::EN_US, "hello"),
        Some("fast".to_string())
...
fn global_catalog_try_resolve_locale_keeps_error_kind_under_replacement() {
    struct BlockingErrorCatalog {
        entered: mpsc::Sender<()>,
        release: Mutex<mpsc::Receiver<()>>,
    }

    impl TranslationCatalog for BlockingErrorCatalog {
        fn resolve_shared(&self, _locale: Locale, _key: &str) -> TranslationResolution {
            TranslationResolution::Missing
        }
    }

    impl Catalog for BlockingErrorCatalog {
        fn lookup_shared(&self, locale: Locale, key: &str) -> Option<Arc<str>> {
            self.get_text_shared(locale, key)
        }
        ...
        fn try_resolve_locale(
            ...
            self.entered.send(()).expect("signal resolver entered");
            self.release
                .lock()
                .expect("lock release receiver")
                .recv()
            ...
            Err(ResolveLocaleError::UnknownLocale {
                requested: requested.unwrap_or_default().to_string(),
            })
        }
    ...
    struct NewErrorCatalog;

    impl TranslationCatalog for NewErrorCatalog {
        fn resolve_shared(&self, _locale: Locale, _key: &str) -> TranslationResolution {
            TranslationResolution::Missing
        }
    }

    impl Catalog for NewErrorCatalog {
        fn lookup_shared(&self, locale: Locale, key: &str) -> Option<Arc<str>> {
            self.get_text_shared(locale, key)
        }
    ...
    let (entered_tx, entered_rx) = mpsc::channel();
    let (release_tx, release_rx) = mpsc::channel();
    catalog.replace(BlockingErrorCatalog {
        entered: entered_tx,
        release: Mutex::new(release_rx),
    });
    ...
    let resolver = thread::spawn(move || {
        resolving_catalog
            .try_resolve_locale(Some("xx_YY"))
            .expect_err("unknown locale should stay an error")
    });

    entered_rx
        .recv_timeout(Duration::from_secs(1))
        .expect("resolver should enter old catalog");
    catalog.replace(NewErrorCatalog);
    release_tx.send(()).expect("release resolver");

    let error = resolver.join().expect("join resolver thread");
    assert_eq!(
        error,
...
#[test]
fn resolve_cli_locale_uses_single_catalog_snapshot_under_global_replacement() {
    struct BlockingCatalog {
        entered: mpsc::Sender<()>,
        release: Mutex<mpsc::Receiver<()>>,
    }
    ...
    impl TranslationCatalog for NewCatalog {
        fn resolve_shared(&self, _locale: Locale, _key: &str) -> TranslationResolution {
            TranslationResolution::Missing
        }
    }

    impl Catalog for NewCatalog {
        fn lookup_shared(&self, locale: Locale, key: &str) -> Option<Arc<str>> {
            self.get_text_shared(locale, key)
        }
        ...
        fn try_resolve_locale(
            ...
            assert_eq!(requested, None);
            Ok(Locale::from_static("fr_FR"))
        }

        fn default_locale(&self) -> Locale {
            Locale::from_static("fr_FR")
        }

        fn available_locales(&self) -> Vec<Locale> {
            vec![Locale::from_static("fr_FR")]
        }
    }
    ...
    let resolver = thread::spawn(move || {
        resolve_locale_from_cli_args(
            &*resolving_catalog,
            Vec::new(),
            "I18N_KIT_TEST_LOCALE_SHOULD_BE_UNSET",
        )
        .expect("locale resolution should use the original catalog snapshot")
    });

    entered_rx
        .recv_timeout(Duration::from_secs(1))
        .expect("resolver should enter the original catalog");
    catalog.replace(NewCatalog);
    release_tx.send(()).expect("release resolver");

    let (locale, cleaned) = resolver.join().expect("join resolver thread");
    assert_eq!(locale, Locale::EN_US);
