...
mod tests {
    ...
    fn managed_resource_temp_roots() -> Vec<std::path::PathBuf> {
        let mut roots = Vec::new();
    ...
    fn managed_resource_test_tempdir(test_name: &str) -> Option<TempDir> {
        for root in managed_resource_temp_roots() {
        ...
        eprintln!(
            "skipping {test_name}: unable to create a usable temp root for managed resource tests"
        );
        None
    }
