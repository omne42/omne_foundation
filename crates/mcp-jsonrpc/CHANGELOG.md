...
### Changed
- `mcp-jsonrpc`：加固了 graceful SSE EOF 重连回归测试：先在初始 SSE 上发送一条真实 notification 证明读侧已处理 body，并移除对重连 GET 套接字逐字节读取的脆弱假设；慢速/macOS CI 不再因为 socket 时序抖动把“EOF 后应重连”的真通过误判成失败。
