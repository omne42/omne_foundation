...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn streamable_http_notification_request_timeout_keeps_client_open() {
    #[derive(Default)]
    struct State {
        notify_post_count: AtomicUsize,
        request_post_count: AtomicUsize,
    ...
    assert!(
        !handle.is_closed(),
        "timed-out notification POST should not close the client"
    );

    let result = client
        .request("ping", serde_json::json!({}))
        .await
        .expect("request should still succeed after notification timeout");
    assert_eq!(result, serde_json::json!({ "ok": true }));
    assert_eq!(state.notify_post_count.load(Ordering::SeqCst), 1);
    assert_eq!(state.request_post_count.load(Ordering::SeqCst), 1);
