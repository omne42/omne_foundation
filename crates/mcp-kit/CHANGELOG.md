...
### Changed
- `mcp-kit`：`ErrorKind` 现在把 `mcp_jsonrpc::ProtocolErrorKind::{Closed,StreamableHttp}` 稳定归类为 `Connection`，避免下游把可重试的 transport/streamable-http 故障误判成协议错误。
...
- `mcp-kit`: `Manager::from_config(...)` 现在在所有 build 模式下都要求 `Config::client()` 先通过校验；非法 `client.roots` / `protocol_version` / `capabilities` 不再只靠 debug-only 断言或首次 initialize 才暴露。
