...
### Fixed
- `mcp-kit`：程序式 `transport=unix` 的相对 `unix_path` 现在会像连接 `cwd` 一样先绑定到调用方传入的 `cwd` 再建立 stable path identity；effective connection identity 和实际 `connect_unix` 不再偷偷依赖进程全局 `current_dir()` 或把相对路径原样交给 OS。
