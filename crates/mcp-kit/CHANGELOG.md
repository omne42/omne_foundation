...
- `mcp-kit`: `ServerConfig` getters for transport-specific fields now return `Option<_>` instead of synthetic defaults on unrelated transports, and `mcpctl list-servers` no longer serializes fake stdio/http values into JSON summaries.
- `mcp-kit`: `Manager::with_server_handler_concurrency(0)` no longer silently clamps to `1`; it now fails fast at construction time so invalid handler settings do not survive until runtime.
- `mcp-kit`: `ServerConfig` now exposes transport-specific view accessors (`stdio_config` / `unix_config` / `streamable_http_config`), and `mcpctl list-servers` now omits unrelated pseudo-default fields instead of serializing misleading transport-mismatched values.
- `mcp-kit`: the new `mcpctl` transport-summary regression test now compiles on Windows too, so the cross-platform `mcp-cross-platform` gate keeps covering the CLI summary contract instead of silently dropping that test target.
...
- `mcp-kit`：`streamable_http` / `jsonrpc` 文档里的 `request_timeout` 说明现在与 `mcp-jsonrpc` 主线实现保持一致：它只约束单次 POST 的非 SSE JSON/error body 读取；对合法 POST-as-SSE 响应，等待响应头和后续流式产出都不再被这个超时误杀。
...
- `mcp-kit`：`SharedManager` 新增的 cold-start gate 回归测试现在把 `AtomicBool` 的测试导入也限定到 Unix 平台，避免 Windows 在对应测试体被条件裁剪后把残留导入视为 `-D warnings` 编译错误；不改变产品行为。
