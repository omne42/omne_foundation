...
### Fixed
- `mcp-kit`：程序式 `transport=unix` 连接现在和 `mcp.json` 装载路径一样，把相对 `unix_path` 绑定到调用方显式传入的连接 `cwd`，并继续复用 stable-path / within-root 校验；不再把同一份 `ServerConfig::unix(\"sock/mcp.sock\")` 因进程环境或调用入口差异解释成另一条 socket。
