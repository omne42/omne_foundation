...
### Fixed
- `mcp-kit`：programmatic `ServerConfig::unix(...)` 的相对 `unix_path` 现在会和 `mcp.json` 加载路径一样，统一按连接 `cwd` / config root 解析成稳定绝对路径；连接身份比较与实际 `connect_unix` 都不再偷偷依赖进程级 `current_dir()`。
