...
- `streamable_http_custom_options`（需要真实 server；演示自定义 `StreamableHttpOptions` 的网络参数，并通过 `connect_streamable_http_split` 直连）：
  ...
  - 注意：默认仍走 `Manager` 的安全边界；当开启 `--follow-redirects` 等高风险参数时，请显式在 Trusted 场景使用
