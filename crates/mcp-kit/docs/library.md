...
- `Config::load` 只接受 canonical `mcp.json v1`；未知字段、缺少 `version`、`mcpServers` wrapper 等都会 fail-closed 报错。
...
1. 在 initialize 里声明你支持的 client capabilities（`Manager::with_capabilities(...)`；这里会在本地 fail-fast 拒绝非对象 JSON）
