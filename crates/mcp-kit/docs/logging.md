...
- `max_parts=0` 在 `mcp-kit` 配置里表示“不限制保留数量”（无限保留）；手动构造 Rust `StdoutLogConfig` 时也接受 `Some(0)` 并按 unlimited 处理
...
在 Rust API（`mcp_jsonrpc::StdoutLog`）里，推荐表达是：
...
- `mcp-kit::StdoutLogConfig { max_parts: Some(0) }` 也会按 unlimited 处理，用来保持与 `mcp.json` 的 `max_parts = 0` 契约一致
