...
- hostnames 会做一次 DNS 解析（带超时；默认 2s，CLI 可用 `--dns-timeout-ms` 调整）；若解析到非公网 IP，会被拒绝（除非同时允许 `allow_private_ips`，而 `localhost` / `*.localhost` 还需要先通过 `allow_localhost` 主机名边界，或直接使用 `Trusted`）
...
- 即使开启了 `allow_private_ips`/`--allow-private-ip`，`dns_check` 仍会继续拦截 always-disallowed 地址，以及“非 localhost host 却解析到 loopback”的可疑 rebinding；只是普通 private/link-local/loopback 目标不再一律拒绝。
