...
- hostnames 会做一次 DNS 解析（带超时；默认 2s，CLI 可用 `--dns-timeout-ms` 调整）；默认策略下，若解析到非公网 IP，会被拒绝
- 开启 `allow_private_ips` / `--allow-private-ip` 后，DNS 侧的“解析到非公网 IP 就拒绝”会整体放开；这包括普通私网、loopback，以及 `0.x.y.z` 这类 host-local 结果
- 因此 `allow_private_ips` 会同时放松连接前 DNS 校验和运行期 `enforce_public_ip` public-IP pinning；它不是“仅放开普通私网、仍保留 loopback/host-local pinning”的开关
...
- 如果你还开启了 `allow_private_ips`，要把它理解为“允许 hostname 最终落到非公网地址”，而不是更窄的私网例外。
