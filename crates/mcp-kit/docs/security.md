...
- hostnames 会做一次 DNS 解析（带超时；默认 2s，CLI 可用 `--dns-timeout-ms` 调整）；若解析到非公网 IP，会被拒绝
- 开启 `allow_private_ips` / `--allow-private-ip` 后，普通私网解析结果会放开；但**非 localhost hostname** 解析到 loopback / host-local 这类“本机地址”时，仍会被拒绝，避免公网域名通过 DNS rebinding 绕回本机
- 只有当目标 hostname 本身就是 `localhost` / `localhost.localdomain` / `*.localhost`，并且同时开启 `allow_localhost` 与 `allow_private_ips` 时，DNS 校验才会接受这类本机地址结果
...
- `allow_private_ips` 只放开普通私网解析结果，不会顺带放开“公网 hostname 解析到 loopback/host-local”的本机重绑定。
