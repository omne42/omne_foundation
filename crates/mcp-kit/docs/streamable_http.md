...
- 遇到空行表示一个 event 结束，此时把累计的 `data` 当作“一条 JSON-RPC 消息”处理后写入内部 JSON-RPC 读循环
...
> 如果拼接后的 `data` 已经是合法 JSON（包括 pretty/multiline JSON），`mcp-jsonrpc` 会先把它压平成单行 JSON，再写回内部按换行分帧的 JSON-RPC 流；无法解析为 JSON 的 payload 才会保持原样写回。
