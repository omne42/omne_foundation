...
### Fixed
- `notify-kit` 的 docs/mdBook 资产检查现在为每次 `mdbook test` 使用独立的临时 `CARGO_TARGET_DIR`；本地 `scripts/docs.sh test` 与 workspace `ci` 也继续共享同一套稳定的 deps 目录准备逻辑，避免复跑时落到共享 target tree 的清理/写入竞态。
...
- `notify-kit::Error` 在聚合 sink 失败时不再把标准 `source()` 链完全截断：aggregate error 现在会把首个 `SinkFailure` 暴露给通用错误检查器，而完整失败列表仍继续通过 `sink_failures()` 保留可检查的 sink 名称、索引和底层错误链。
...
- `notify-kit::env` 的 helper 边界现在在代码和 README 中都显式声明为 convenience wiring，而不是 crate root `Hub` / `Sink` foundation 的默认接线协议。
