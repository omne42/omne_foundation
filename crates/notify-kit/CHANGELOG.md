...
- `Event::new_structured` / `with_*_text` 现在会在缺少显式 plain fallback 时自动生成稳定字符串投影，避免 `Bark`、`PushPlus`、`ServerChan`、`Feishu post` 这类仍消费 plain payload 的 sink 把 structured-only title/body/tag 静默丢空。
