...
### Fixed
- `GenericWebhookSink` 现在直接复用 `http-kit` 的 host allowlist matcher；严格模式和默认推导边界下的 `allowed_hosts` 不再只做精确字符串比较，从而和 workspace 统一的“父域名允许子域名、IP literal 必须精确匹配”语义保持一致。
