...
- token-bearing 请求始终强制做 DNS 公网 IP 校验；`with_public_ip_check(false)` 不会放宽 GitHub token 的出站边界
- 如果显式信任自定义 GitHub API base，也必须保持公网 IP 校验；localhost / 私网 IP literal 会被直接拒绝，解析到私网/回环地址的伪装域名也不能绕过发送期校验
...
`Event` 的 `title()` / `body()` / `tags()` 内容由上层业务提供，可能包含：
