...
- `local_image_root(s)` 是最终读取边界；如果 Markdown 里写的是相对图片路径（例如 `![img](./exported-images/a.png)`），还必须显式配置绝对 `local_image_base_dir`，并且解析后的最终路径仍然必须落在这些 root allowlist 之内
...
- 只会读取显式 `local_image_root(s)` 之下的文件；相对路径也只是借助显式 `local_image_base_dir` 做解析，最终仍会按 root allowlist 校验，超出 root、`..` 逃逸、symlink 组件和其他特殊路径都会 fail closed
