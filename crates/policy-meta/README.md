...
- `policy_meta::contract` 只承载稳定的策略契约语义；crate 根只 re-export 这些核心类型
- `policy_meta::artifacts` 承载纯生成 helper 与对应 typed error；上游如果遇到 schema/TS 生成器行为漂移，会得到可处理错误而不是 panic
...
- `policy-meta` 库本身只暴露共享策略语义，以及放在 `artifacts` 子模块下的纯导出结果
