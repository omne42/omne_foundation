...
#[cfg(unix)]
const BOOTSTRAP_LOCK_DIR_UNIX: &str = "/tmp/.runtime-assets-kit-bootstrap-locks";
...
pub(crate) struct BootstrapTransactionGuard {
...
pub(crate) fn lock_bootstrap_transaction(root: &Path) -> io::Result<BootstrapTransactionGuard> {
...
fn bootstrap_lock_dir() -> PathBuf {
    ...
    #[cfg(not(unix))]
    {
        std::env::temp_dir().join(".runtime-assets-kit-bootstrap-locks")
    }
}
