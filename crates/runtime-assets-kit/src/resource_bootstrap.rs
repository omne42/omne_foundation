...
use omne_systems_fs_primitives::{Dir, MissingRootPolicy};
...
pub(crate) fn bootstrap_text_resources(
    root: &Path,
...
pub(crate) fn rollback_created_resources(report: &BootstrapReport) -> io::Result<()> {
...
fn remove_created_file(base: &Dir, base_path: &Path, path: &Path) -> io::Result<()> {
    ...
    omne_systems_fs_primitives::remove_file_or_symlink_at(&parent, Path::new(leaf))
}
...
fn open_rollback_directory(
    ...
    for component in components {
        ...
        directory = omne_systems_fs_primitives::open_directory_component(&directory, component)
            .map_err(|error| {
                map_rollback_access_error(&directory, component, &current_path, error)
            })?;
    }
    Ok(directory)
}
...
mod tests {
    ...
    fn bootstrap_creates_missing_root_with_secure_open() {
        ...
        let report = bootstrap_text_resources(&root, &manifest).expect("bootstrap nested root");
