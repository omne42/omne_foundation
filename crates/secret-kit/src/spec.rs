...
pub(super) fn optional_query_value(query: &QueryParameters, key: &str) -> Option<String> {
    query
        .get(key)
...
pub(crate) fn build_secret_command(spec: &SecretSpec) -> Option<SecretCommand> {
    providers::build_provider_command(spec)
}
...
pub(super) fn ensure_allowed_query_parameters(
    provider: &str,
...
pub(super) fn reject_option_like_cli_argument(
    provider: &str,
    field: &str,
    value: &str,
) -> Result<()> {
...
pub(super) fn decode_spec_component(raw: &str) -> Result<String> {
