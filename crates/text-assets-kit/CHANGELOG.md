...
- Marked `text-assets-kit::lazy_value` itself as a hidden compatibility module so the crate root stays the only supported deprecated shim entry while callers migrate to explicit runtime-owned boundaries.
