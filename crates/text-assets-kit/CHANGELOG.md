...
- Removed the crate-root `LazyValue` / `LazyInitError` compat exports entirely; `text-assets-kit` no longer presents a generic blocking lazy-init surface as part of its runtime adapter boundary, and the old lazy shim now remains internal to crate tests instead of any public API path.
