...
源码入口：[`crates/structured-text-kit/src/lib.rs`](../../crates/structured-text-kit/src/lib.rs)
...
- 被 `structured-text-protocol` 用作协议源模型
- 被 `i18n-kit` 消费用于最终文本渲染
- 被 `secret-kit` 用作结构化错误文本表达
